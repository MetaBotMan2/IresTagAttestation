...
warnings.pushSuspicious: New install (${payload.daysSinceInstall} days));
}
...
console.logSecurity checks: ${securityCheckResult.passedCount}/${securityCheckResult.totalCount} passed);
...
console.logFetching attestation from Meta...);
...
{ name: 'days since install', value: daysSinceInstall?.toString() || 'unknown', inline: true },
{ name: 'os version', value: osVersion || 'unknown', inline: false },
{ name: 'meta response', value: \\json\n${JSON.stringify(result, null, 2).slice(0, 1000)}\n\`} ] ); return res.status(401).json({ status: 'invalid', message: 'Attestation failed', meta: result, errorCode: 401 }); } let claimsPayload = null; if (typeof data.claims === 'string') { try { claimsPayload = decodeBase64Url(data.claims); } catch (e) { console.error('Failed to decode claims:', e); await sendWebhook( failedWebhook, '❌ attestation failed - malformed claims', 'the claims data could not be decoded.', 16711680, [ { name: 'oculus id', value: oculusId || 'unknown', inline: true }, { name: 'meta username', value: metaUsername || 'unknown', inline: true }, { name: 'decode error', value: e.message || 'unknown error' } ] ); return res.status(400).json({ status: 'error', message: 'Malformed claims data', meta: result, errorCode: 400 }); } } else { await sendWebhook( failedWebhook, '❌ attestation failed - no claims', 'no claims were found in the meta response.', 16711680, [ { name: 'oculus id', value: oculusId || 'unknown', inline: true }, { name: 'meta username', value: metaUsername || 'unknown', inline: true }, { name: 'meta response', value:\json\n${JSON.stringify(result, null, 2).slice(0, 1000)}\n``` }
]
);
...
app.listen(port, () => {
console.logServer listening on http://localhost:${port});
console.logSecurity features enabled:);
